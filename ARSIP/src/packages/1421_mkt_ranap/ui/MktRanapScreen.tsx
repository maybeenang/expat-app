...
import {exportMktRanap, getMktRanaps} from '../services/mktRanapService';
...
const MktRanapScreen = ({navigation}) => {
  ...
  const [data, setData] = useState<MktRanapResponse | null>(null);
  ...
  const handleExport = async (type: ExportType) => {
    ...
    try {
      const result = await exportMktRanap(
        type,
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      const result = await getMktRanaps(
        selectedFilter,
        selectedYear.toString(),
        selectedMonthId,
      );
