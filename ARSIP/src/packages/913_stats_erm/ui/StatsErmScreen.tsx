...
import {getStatsErms, exportStatsErm} from '../services/statsErmService';
import {
  BottomSheetModal,
...
import {columns} from '../data/statsErmColumn';
import ChartList from '../../../components/ChartList';
import {chartDataKeys} from '../data/statsErmChartDataKeys';

const StatsErmScreen = ({navigation}) => {
  ...
  const [data, setData] = useState<StatsErmResponse | null>(null);
  ...
  const handleExport = async (type: ExportType) => {
    ...
    try {
      const result = await exportStatsErm(
        type,
        selectedFilterId,
        isShowGraphic ? '1' : '0',
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      const result = await getStatsErms(
        selectedFilterId,
        selectedYear.toString(),
        selectedMonthId,
