...
import {exportMktRajal, getMktRajals} from '../services/mktRajalService';
...
import MonthBottomSheetView from '../../../components/MonthBottomSheetView';
import YearBottomSheetView from '../../../components/YearBottomSheetView';
...
const MktRajalScreen = ({navigation}) => {
  ...
  const [data, setData] = useState<MktRajalResponse | null>(null);
  ...
  const columns = [
    ...
    {label: 'Checkout Time', field: 'checkout_time'},
    ...
    {label: 'Ruang Rawat', field: 'ruang_rawat'},
    {label: 'Nama Poli', field: 'nama_poli'},
  ];

  const handleExport = async (type: ExportType) => {
    setLoadingDialog(true);
    try {
      const result = await exportMktRajal(
        type,
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      const result = await getMktRajals(
        selectedFilter,
        selectedYear.toString(),
        selectedMonthId,
      );
    ...
    fetchData();
  }, [selectedFilter, selectedYear, selectedMonthId]);
