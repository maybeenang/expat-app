...
import {
  getBedRekap,
  exportBedRekap,
  getFilterDataBedRekap,
} from '../services/bedRekapService';
import {
  BottomSheetModal,
...
const BedRekapScreen = ({navigation}) => {
  ...
  const [data, setData] = useState<BedRekapResponse | null>(null);
  ...
  const columns = [
    {label: 'ID Dept', field: 'id_dept', width: 100},
    {label: 'ID Kamar', field: 'id_kamar', width: 100},
    {label: 'ID Bed', field: 'id_bed', width: 100},
    {label: 'Bed Occupied Date', field: 'bed_occupied_date', width: 150},
    {
      label: 'Nama Ruangan Perawatan',
      field: 'nama_ruangan_perawatan',
      width: 200,
    },
    {label: 'Nama Bed', field: 'nama_bed', width: 150},
    {label: 'Kelas', field: 'kelas', width: 120},
    {label: 'Siranap Perawatan', field: 'siranap_perawatan', width: 180},
    {label: 'Rsonline Perawatan', field: 'rsonline_perawatan', width: 180},
  ];
  ...
  const filterDataTypes = useMemo(
    () => ['nama_ruangan_perawatan', 'nama_bed', 'kelas'],
    [],
  );
  ...
  const handleExport = async (type: ExportType) => {
    ...
    try {
      const result = await exportBedRekap(
        type,
        selectedFilterId,
        '0',
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      const result = await getBedRekap(
        selectedFilterId,
        selectedYear.toString(),
        selectedMonthId,
    ...
    const fetchFilterData = async () => {
      try {
        filterDataTypes.forEach(async filterDataType => {
          const result = await getFilterDataBedRekap(filterDataType);
