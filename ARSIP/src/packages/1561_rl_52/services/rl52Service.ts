...
export const getRl52 = async (
  ...
  try {
    ...
    // API call
    const response = await api.get<RL52Response>('/1561_rl_52', {
      params,
      headers: {
    ...
    console.log('Error fetching rl 52:', error);
    return null;
  }
};

export const exportRL52 = async (
  type: ExportType,
  filter: string,
  graphic: string,
  tahun?: string,
  ...
  try {
    ...
    const response = await api.get<ExportResponse>(`/1561_rl_52/${type}`, {
      params,
      headers: {
        Authorization: `Bearer ${token}`,
      },
    ...
    console.log('Error exporting rl 52:', error);
